...
def test_analyze_all_runs_every_check(monkeypatch):
    ...
    monkeypatch.setattr(
        ...
        {c: (lambda e, c=c: (seen.append(c) or [])) for c in (22, 119, 125, 190, 327, 338, 362, 367, 369, 377, 401, 415, 416, 476, 78, 134, 676, 732, 787)},
    )
    rep = analyzer.analyze("bin", "all", engine_factory=_fake_factory)
    assert seen == [22, 119, 125, 190, 327, 338, 362, 367, 369, 377, 401, 415, 416, 476, 78, 134, 676, 732, 787]
    assert rep.findings == []
...
class ArchAwareFakeEngine(FakeEngine):
    ...
    _RUNNABLE = (78, 190, 327, 338, 362, 367, 377, 676)
...
def test_analyze_skips_unsupported_arch_checks(monkeypatch):
    ...
    # Only the arch-agnostic checks actually ran (190, 327, 338, 362, 367, 377, 78, 676).
    assert seen == [190, 327, 338, 362, 367, 377, 78, 676]
    # The skipped register-level checks are recorded on the report.
    assert rep.skipped_checks == [22, 119, 125, 369, 401, 415, 416, 476, 134, 732, 787]
    assert rep.to_dict()["skipped_checks"] == [22, 119, 125, 369, 401, 415, 416, 476, 134, 732, 787]
