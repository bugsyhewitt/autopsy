...
def test_analyze_all_runs_every_check(monkeypatch):
    ...
    monkeypatch.setattr(
        ...
        {c: (lambda e, c=c: (seen.append(c) or [])) for c in (119, 190, 338, 369, 377, 415, 416, 78, 134, 676, 732, 787)},
    )
    rep = analyzer.analyze("bin", "all", engine_factory=_fake_factory)
    assert seen == [119, 190, 338, 369, 377, 415, 416, 78, 134, 676, 732, 787]
    assert rep.findings == []
...
class ArchAwareFakeEngine(FakeEngine):
    ...
    377, 676) run; the register-level checks are reported as skipped.
    """

    _RUNNABLE = (78, 190, 338, 377, 676)
...
def test_analyze_skips_unsupported_arch_checks(monkeypatch):
    ...
    # Only the arch-agnostic checks actually ran (190, 338, 377, 78, 676).
    assert seen == [190, 338, 377, 78, 676]
    # The skipped register-level checks are recorded on the report.
    assert rep.skipped_checks == [119, 369, 415, 416, 134, 732, 787]
    assert rep.to_dict()["skipped_checks"] == [119, 369, 415, 416, 134, 732, 787]
