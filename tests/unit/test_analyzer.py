...
def test_analyze_all_runs_every_check(monkeypatch):
    ...
    monkeypatch.setattr(
        ...
        {c: (lambda e, c=c: (seen.append(c) or [])) for c in (119, 190, 415, 416, 78, 134, 676, 787)},
    )
    rep = analyzer.analyze("bin", "all", engine_factory=_fake_factory)
    assert seen == [119, 190, 415, 416, 78, 134, 676, 787]
    assert rep.findings == []
...
def test_analyze_skips_unsupported_arch_checks(monkeypatch):
    ...
    # Only the arch-agnostic checks actually ran (78, 190, 676).
    assert seen == [190, 78, 676]
