...
def test_analyze_all_runs_every_check(monkeypatch):
    ...
    monkeypatch.setattr(
        ...
        {c: (lambda e, c=c: (seen.append(c) or [])) for c in (22, 119, 125, 190, 338, 362, 367, 369, 377, 401, 415, 416, 476, 78, 134, 676, 732, 787)},
    )
    rep = analyzer.analyze("bin", "all", engine_factory=_fake_factory)
    assert seen == [22, 119, 125, 190, 338, 362, 367, 369, 377, 401, 415, 416, 476, 78, 134, 676, 732, 787]
    assert rep.findings == []
