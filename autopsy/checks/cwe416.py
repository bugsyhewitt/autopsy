...
slot into rax (or another GPR) before each use and copies it into rdi before
the free call. We therefore track which stack slot the malloc result lives in,
follow reg<-slot reloads, and recognize free's argument even when it arrives
via ``mov rdi, rax``. Capstone renders small displacements in decimal
(``[rbp - 8]``) and larger ones in hex; both are normalized.
"""
...
_DEREF_BASE = re.compile(r"\[(r[a-z0-9]+)")
    ...
    for idx, insn in enumerate(insns):
        ...
        if mn == "call":
            ...
            if target == "free" and ptr_slot is not None and free_addr is None:
                # Confirm free's arg (rdi) aliases our slot.
