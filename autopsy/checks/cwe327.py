...
_BROKEN: dict[str, tuple[str, str]] = {
    ...
    "MD2_Init": (
        "MD2 is a broken hash (preimage and collision attacks since 2009)",
        "SHA-256 / SHA-3",
    ),
    ...
    "MD5": (
        "MD5 is a broken hash (chosen-prefix collisions are practical;"
        " not safe for signatures or auth)",
    ...
    "DES_encrypt1": (
        "single DES has a 56-bit key; brute-forced since 1998",
        "AES-128/AES-256 (GCM or CTR+HMAC)",
    ),
    "DES_encrypt2": (
        "single DES has a 56-bit key; brute-forced since 1998",
        "AES-128/AES-256 (GCM or CTR+HMAC)",
    ...
    "DES_encrypt3": (
        "3DES has a 64-bit block; Sweet32 makes it unsafe for nontrivial volume",
        "AES-128/AES-256 (GCM)",
    ),
    "DES_ecb_encrypt": (
        "single DES has a 56-bit key; brute-forced since 1998",
    ...
    "DES_set_key": (
        "DES key schedule (uses single DES); single DES is brute-forceable",
        "AES-128/AES-256 (GCM or CTR+HMAC)",
    ),
    ...
    "RC2_encrypt": (
        "RC2 has a 64-bit block and known related-key attacks; long deprecated",
        "AES-128/AES-256 (GCM)",
    ),
    "RC2_cbc_encrypt": (
        "RC2 has a 64-bit block and known related-key attacks; long deprecated",
    ...
    "EVP_md2": (
        "EVP_md2() selects the broken MD2 hash",
        "EVP_sha256 / EVP_sha3_256 / EVP_blake2b512",
    ),
    "EVP_md4": (
        "EVP_md4() selects the broken MD4 hash",
        "EVP_sha256 / EVP_sha3_256 / EVP_blake2b512",
    ),
    "EVP_md5": (
        "EVP_md5() selects the broken MD5 hash",
        "EVP_sha256 / EVP_sha3_256 / EVP_blake2b512",
    ),
    "EVP_md5_sha1": (
        "EVP_md5_sha1() selects a combined MD5+SHA1 hash; both halves are broken",
        "EVP_sha256 / EVP_sha3_256",
    ),
    "EVP_sha1": (
        "EVP_sha1() selects the broken SHA-1 hash",
        "EVP_sha256 / EVP_sha3_256 / EVP_blake2b512",
    ),
    "EVP_des_cbc": (
        "EVP_des_cbc() selects single DES; 56-bit key brute-forced since 1998",
        "EVP_aes_128_gcm / EVP_aes_256_gcm",
    ),
    "EVP_des_ecb": (
        "EVP_des_ecb() selects single DES; ECB also leaks plaintext structure",
        "EVP_aes_128_gcm / EVP_aes_256_gcm",
    ),
    "EVP_des_cfb": (
        "EVP_des_cfb() selects single DES; 56-bit key brute-forced since 1998",
        "EVP_aes_128_gcm / EVP_aes_256_gcm",
    ),
    "EVP_des_ofb": (
        "EVP_des_ofb() selects single DES; 56-bit key brute-forced since 1998",
        "EVP_aes_128_gcm / EVP_aes_256_gcm",
    ),
    "EVP_des_ede": (
        "EVP_des_ede() selects 2-key 3DES; 64-bit block (Sweet32) and reduced security margin",
        "EVP_aes_128_gcm / EVP_aes_256_gcm",
    ),
    "EVP_des_ede_cbc": (
        "EVP_des_ede_cbc() selects 2-key 3DES; 64-bit block (Sweet32) and reduced security margin",
        "EVP_aes_128_gcm / EVP_aes_256_gcm",
    ),
    "EVP_des_ede3": (
        "EVP_des_ede3() selects 3-key 3DES; 64-bit block makes it Sweet32-vulnerable",
        "EVP_aes_128_gcm / EVP_aes_256_gcm",
    ),
    "EVP_des_ede3_cbc": (
        "EVP_des_ede3_cbc() selects 3-key 3DES; 64-bit block makes it Sweet32-vulnerable",
        "EVP_aes_128_gcm / EVP_aes_256_gcm",
    ),
    "EVP_rc4": (
        "EVP_rc4() selects RC4; practical biases (NOMORE 2015; RFC 7465 prohibits in TLS)",
        "EVP_aes_128_gcm / EVP_chacha20_poly1305",
    ),
    "EVP_rc4_40": (
        "EVP_rc4_40() selects 40-bit RC4; export-grade key length, trivially broken",
        "EVP_aes_128_gcm / EVP_chacha20_poly1305",
    ),
    "EVP_rc2_cbc": (
        "EVP_rc2_cbc() selects RC2; 64-bit block and related-key attacks",
        "EVP_aes_128_gcm / EVP_aes_256_gcm",
    ),
    "EVP_rc2_40_cbc": (
        "EVP_rc2_40_cbc() selects 40-bit RC2; export-grade, trivially broken",
        "EVP_aes_128_gcm / EVP_aes_256_gcm",
    ),
    "EVP_bf_cbc": (
        "EVP_bf_cbc() selects Blowfish; 64-bit block (Sweet32)",
        "EVP_aes_128_gcm / EVP_aes_256_gcm",
    ),
    "EVP_bf_ecb": (
        "EVP_bf_ecb() selects Blowfish; 64-bit block (Sweet32) and ECB plaintext leakage",
        "EVP_aes_128_gcm / EVP_aes_256_gcm",
    ),
    ...
    "crypt": (
        "crypt() defaults to DES-based password hashing;"
        " even its MD5/SHA variants are too fast for password storage",
        "argon2 / bcrypt / scrypt (or libxcrypt with $argon2id$/$7$/$6$ setting)",
    ),
    "crypt_r": (
        "crypt_r() inherits crypt()'s legacy algorithm selection;"
        " the DES and MD5 variants are unsafe",
        "argon2 / bcrypt / scrypt (or libxcrypt with $argon2id$/$7$/$6$ setting)",
    ),
}
